...
def parse_args():
    ...
    parser.add_argument('--checkpoint_dir', type=str, required=True,
                       help='Directory containing trained GNN model')
    ...
    parser.add_argument('--test_size', type=int, default=100,
                       help='Number of test instances (default: 100)')
    parser.add_argument('--n_items_min', type=int, default=10,
                       help='Minimum items per instance')
    parser.add_argument('--n_items_max', type=int, default=50,
                       help='Maximum items per instance')
    ...
    parser.add_argument('--n_samples', type=int, default=200,
                       help='Number of samples for GNN sampling strategy')
    parser.add_argument('--temperature', type=float, default=0.8,
                       help='Sampling temperature')

    # Random baseline parameters
    parser.add_argument('--random_attempts', type=int, default=100,
                       help='Attempts for random baseline')
...
def get_ortools_stats(test_dataset) -> Dict:
    ...
    times = [inst.solve_time for inst in test_dataset.instances
             if inst.solve_time is not None]
...
def create_comparison_table(all_stats: List[Dict]) -> None:
    ...
    ortools_time = next((s['mean_time'] for s in all_stats
                        if s['method'] == 'OR-Tools (Exact)'), None)
...
def plot_comparison(all_stats: List[Dict], output_dir: str):
    ...
    for i, (m, g) in enumerate(zip(methods, gaps)):
    ...
    for stats in methods_to_plot:
        if 'results' in stats:
            gaps_list = [r['optimality_gap'] for r in stats['results']
    ...
    if gap_distributions:
        ...
        for patch, color in zip(bp['boxes'], colors[:len(gap_distributions)]):
    ...
    for i, (g, t, m, c) in enumerate(zip(gaps, times, methods, colors[:len(methods)])):
        ax.scatter(t, g, s=300, alpha=0.7, color=c, edgecolor='black', linewidth=2)
        ax.annotate(m, (t, g), fontsize=10, ha='center', va='bottom',
...
def main():
    ...
    # Best accuracy
    best_accuracy = min(all_stats, key=lambda x: x['mean_gap'] if 'OR-Tools' not in x['method'] else float('inf'))
