...
from knapsack_gnn.data.generator import KnapsackDataset, KnapsackSolver
...
def analyze_instance(instance_idx: int,
                     dataset,
                     model,
                     sampler,
...
def print_analysis(analysis: dict):
    ...
    print(f"  Value/Weight ratio range: [{np.min(analysis['value_weight_ratios']):.2f}, {np.max(analysis['value_weight_ratios']):.2f}]")
    ...
    print(f"\n⚖️ Capacity Usage:")
    print(f"  Optimal: {analysis['optimal_weight']:.0f}/{analysis['capacity']} ({analysis['optimal_capacity_usage']*100:.1f}%)")
    print(f"  Predicted: {analysis['predicted_weight']:.0f}/{analysis['capacity']} ({analysis['predicted_capacity_usage']*100:.1f}%)")
    ...
    if analysis['n_wrong_included'] > 0:
        ...
        for idx in analysis['wrong_included_indices']:
            print(f"    Item {idx}: w={analysis['weights'][idx]:.0f}, v={analysis['values'][idx]:.0f}, " +
    ...
    if not analysis['optimal_value_matches']:
        print(f"    ⚠️ MISMATCH: Stored={analysis['optimal_value']}, Recalculated={analysis['recalculated_optimal']}")
...
def visualize_instance(analysis: dict, save_path: str = None):
    ...
    width = 0.35
    ax1.bar(x - width/2, analysis['optimal_solution'], width, label='Optimal', alpha=0.7, color='green')
    ax1.bar(x + width/2, analysis['predicted_solution'], width, label='Predicted', alpha=0.7, color='blue')
...
Value Check:
  {'✓' if analysis['optimal_value_matches'] else '⚠️ MISMATCH'}
    ...
    fig.suptitle(f'Outlier Instance #{analysis["instance_idx"]} - Detailed Analysis',
...
def compare_with_normal(outlier_analysis: dict,
                       normal_indices: list,
                       dataset,
                       sampler,
                       strategy: str = 'sampling') -> dict:
    ...
    print(f"  Normal avg: {np.mean([a['optimal_capacity_usage'] for a in normal_analyses])*100:.1f}%")
...
def main():
    ...
    for outlier_idx, outlier_gap in outliers:
        ...
        analysis = analyze_instance(outlier_idx, test_dataset, model, sampler, args.strategy, args.device)
        ...
        if normal_indices:
            comparison = compare_with_normal(analysis, normal_indices, test_dataset, sampler, args.strategy)
